...
| 서버→토스 API 호출 | `x-anon-key` 헤더 (hash 인증) | ❌ |

- 익명키는 **미니앱마다 고유**하고, **같은 사용자는 같은 미니앱에서 항상 같은 값**을 받는다.
  기기가 아니라 **토스 계정 기준**이므로 기기 변경·재설치에도 유지된다.
- **hash 인증으로 토스 서버 API 도 호출한다** — 프로모션·스마트 발송(알림)·토스페이·인앱결제가
  토스 로그인 없이 익명키만으로 지원된다. (내부 식별 전용이 아니다)
- 발급값은 서버에서 anon-key verify API 로 검증한다.
