...
토스 미니앱으로 제공하는 **영상 자막 생성 서비스**. 사용자가 영상을 올리면 대본을 뽑아 주고,
사용자가 그 대본을 고쳐 확정하면 자막 파일(md)로 돌려준다.
...
| 모듈 | 상태 |
|---|---|
| `auth` 익명키 로그인 · JWT 발급/게이트/갱신 | 구현 완료 |
| `payment` 결제·지급 원장 (토스 IAP mTLS 검증) | 구현 완료 |
| `credit` 횟수권 잔량 | 구현 완료 |
| `subscription` 구독 계약·웹훅 | 구현 완료 |
| `subtitle` 자막 작업 · 워커 큐 연동 | 구현 완료 |
| `bootstrap` 진입 집계 | 구현 완료 |

**아직 붙지 않은 것** — 부르는 쪽이 생길 때 만든다.

- **객체 스토리지 어댑터** — `SignedUrlIssuer`·`StorageInspector` 는 부르면 실패하는 임시 구현이다
- **이용권 구현체** — `PaymentUsagePort` 는 전부 거부하는 임시 어댑터다 (이용 티켓 애그리거트 이연)
- **워커** — 음성 인식과 자막 산출을 하는 온프레미스 처리 주체. 이 레포 밖이고 큐로만 닿는다
- **프론트엔드** — 아직 이 레포에 없다
...
4. [`backend/docs/new-domain/subtitle/subtitle-v3.md`](backend/docs/new-domain/subtitle/subtitle-v3.md)
   — 정의서가 어떻게 생겼는지 보는 가장 좋은 예. 이 서비스가 실제로 파는 것이기도 하다
